// three-slicer-viewer/settings — the catalog-free settings surface (three-slicer/settings re-exports it and adds the vendor catalog).
 ...
 * merges `preset`. A null preset applies nothing and returns the settings unchanged — a failed lookup must
 * not half-clear the previous pick.
 */
export function applyPreset(settings: SlicerSettings | null | undefined, preset: Partial<SlicerSettings> | null | undefined, keys: readonly string[] | null | undefined): SlicerSettings

/**
 * The schema keys deriveKernelParams consumes — the distinct "From setting" column of engine/PARAMS.md,
 * generated by the same probe that generates the table (so it cannot drift from the real mapping).
 */
export const kernelSettingKeys: readonly string[]

/**
 * The keys of `settings` that deriveKernelParams does NOT read — they are accepted and silently have no
 * effect on an FFF slice. Scope is the FFF derivation only: a key listed here can still be meaningful to
 * the SLA derivation, the viewer, or project metadata. Sorted.
 */
export function ignoredKernelSettings(settings: SlicerSettings | null | undefined): string[]
...
// ---- The vendor catalog: NOT in this package ---------------------------------
// printerKeys, printersByVendor, printerTechByVendor, resinCatalog, resinSettingsFor, printerSettings,
// printerDefaultPreset, processPresets and filamentPresets are OrcaSlicer's profile data (AGPL) and live in
// `three-slicer/settings`. The viewer takes them through its `catalog` prop (see `Catalog` in viewer.d.ts);
// the interface shapes below are what such a catalog's lazily loaded halves return, kept here so a host can
// type its own.
