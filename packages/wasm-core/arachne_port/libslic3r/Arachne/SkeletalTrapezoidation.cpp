...
SkeletalTrapezoidation::Beading SkeletalTrapezoidation::interpolate(const Beading& left, double ratio_left_to_whole, const Beading& right, coord_t switching_radius)
{
    ...
    // WASM port fix: size()-1 must be subtracted **after casting to the signed coord_t**.
    //  When toolpath_locations is empty, size_t(0)-1 is 2^64-1 on x86-64 -> -1 as int64_t, which the
    //  `next_inset_idx < 0` guard below catches; but on wasm32 size_t is 32-bit, so it is 2^32-1 -> +4294967295 as int64_t
    //  (positive!). The guard is defeated and left.toolpath_locations[4294967295] (a ≈34GB offset) is read,
    //  killing the kernel with "memory access out of bounds". A defect upstream cannot see because it is 64-bit only.
    ...
    // right can be shorter than left for the same reason — the new_ratio expression below indexes
    // right.toolpath_locations directly with next_inset_idx, so it is guarded as well (the same class of OOB).
