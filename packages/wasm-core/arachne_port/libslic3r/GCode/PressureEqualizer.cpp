...
#include <boost/algorithm/string.hpp>   // STAGE-8 PORT MOD: boost::contains/starts_with/ends_with (was transitive via real Utils.hpp; header-only, no link)
#include "../libslic3r.h"
...
// STAGE-8 PORT MOD: GCodeConfig reads replaced by injected params (see .hpp).
PressureEqualizer::PressureEqualizer(double filament_diameter_mm, double max_slope_mm3_s2,
                                     double segment_length_mm, bool use_relative_e, bool external_perimeter_only)
    : m_use_relative_e_distances(use_relative_e)
{
    ...
    for (double r : { filament_diameter_mm }) {   // STAGE-8: single injected filament diameter
    ...
    if(max_slope_mm3_s2 > 0){
		m_max_volumetric_extrusion_rate_slope_positive = float(max_slope_mm3_s2) * 60.f * 60.f;
    	m_max_volumetric_extrusion_rate_slope_negative = float(max_slope_mm3_s2) * 60.f * 60.f;
    	m_max_segment_length = float(segment_length_mm);
        m_extrusion_rate_smoothing_external_perimeter_only = external_perimeter_only;
    }
...
// STAGE-8 PORT MOD: LayerResult process_layer(LayerResult&&) removed (needs GCode.hpp LayerResult).
// Added process_to_string(): load+equalize a whole-layer g-code string, then emit via the original
// output_gcode_line() machinery (uses GCodeG1Formatter). Algorithm unchanged.
std::string PressureEqualizer::process_to_string(const std::string &gcode)
{
    this->process_layer(gcode);                 // load lines + adjust volumetric rate (original logic)
    output_buffer_length = 0; output_buffer_prev_length = 0;
    for (size_t i = 0; i < m_gcode_lines.size(); ++i)
        output_gcode_line(i);                   // original emitter (GCodeG1Formatter)
    m_gcode_lines.clear();
