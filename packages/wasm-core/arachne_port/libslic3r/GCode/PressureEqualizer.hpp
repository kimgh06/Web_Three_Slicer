...
#include "../ExtrusionEntity.hpp"   // STAGE-8 PORT MOD: ExtrusionRole (was transitive via GCode.hpp)
...
public:
    ...
    // STAGE-8 PORT MOD: original ctor takes GCodeConfig (PrintConfig machinery). Replaced with direct
    // param injection (task-sanctioned). Fields read: filament_diameter, max_volumetric_extrusion_rate_slope,
    // ..._segment_length, use_relative_e_distances, extrusion_rate_smoothing_external_perimeter_only.
    explicit PressureEqualizer(double filament_diameter_mm, double max_slope_mm3_s2,
                               double segment_length_mm, bool use_relative_e, bool external_perimeter_only);
    ~PressureEqualizer() = default;
    // STAGE-8 PORT ADD: process a whole-layer g-code string and return the pressure-equalized string.
    std::string process_to_string(const std::string &gcode);

    // Process a next batch of G-code lines.
    // STAGE-8 PORT MOD: LayerResult process_layer(LayerResult&&) overload removed (needs GCode.hpp
    // LayerResult). process_to_string() above is used instead for whole-layer string processing.
private:
