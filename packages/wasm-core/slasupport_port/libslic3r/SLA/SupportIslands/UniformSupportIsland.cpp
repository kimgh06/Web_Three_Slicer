...
#include <libslic3r/Utils.hpp> // PORT-EDIT: ScopeGuard — transitive in Prusa, explicit under this include universe
...
// PORT-EDIT: the Orca-generation Line has no perp_signed_distance_to; this is upstream
//  Prusa Line.cpp:67 verbatim, as a free helper (the method body only, `line` bound to the argument).
double perp_signed_distance_to_line(const Line &line, const Point &point) {
...
{  
    ...
    for (size_t i = 0; i < supports.size(); i++) {
        ...
        // PORT-EDIT: Orca-generation ClipperUtils has no (Polygon, ExPolygon) overload; wrap the
        //  subject in Polygons to hit its (Polygons, ExPolygon) one — same clip, same result.
...
Field create_field(const Slic3r::ExPolygon &island, float offset_delta, const std::vector<bool>& is_outline)
{
    ...
    for (size_t island_line_index = 0; island_line_index < island_lines.size(); ++island_line_index) {
        ...
        do {
            ...
            double distance = perp_signed_distance_to_line(island_line, offset_middle); // PORT-EDIT: free helper, see top of file
