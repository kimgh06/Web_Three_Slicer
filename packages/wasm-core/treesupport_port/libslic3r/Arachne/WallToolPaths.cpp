...
// STAGE-7 PORT MOD: removed make_paths_params(layer, PrintObjectConfig, PrintConfig) definition.
// It reads PrintConfig/PrintObjectConfig fields (heavy config machinery, stubbed out). The WASM
// port fills WallToolPathsParams manually from kernel params. The .hpp declaration is left intact
// (references to forward-declared config classes compile fine) but is never called/linked.
