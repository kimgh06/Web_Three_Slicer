...
{
    // STAGE-8 PORT MOD: factory trimmed to the ported patterns only. The original switch references
    // every Fill subclass (FillRectilinear/Adaptive/Lightning/Tpms/... vtables), most of which are not
    // ported (heavy deps). Unported patterns return nullptr; the kernel falls back to its own infill.
    switch (type) {
    ...
    // (stage 16) tree-support fill patterns: SupportParameters selects base=ipSupportBase (or
    // ipHoneycomb) and interface=ipRectilinear/ipSupportBase. FillSupportBase/FillRectilinear live in
    // FillRectilinear.hpp (linked). Without these, new_from_type returned nullptr -> null filler deref.
...
// STAGE-8 PORT MOD: removed Fill::new_from_type(const std::string&) body. It maps a config enum
// STRING to InfillPattern via ConfigOptionEnum<>::get_enum_values() (Config.hpp machinery, stubbed).
// The port calls the InfillPattern overload directly, so this string factory is unused.
Fill* Fill::new_from_type(const std::string &) { return nullptr; }
...
bool Fill::use_bridge_flow(const InfillPattern type)
{
	...
	if (cached.empty()) {
		...
		for (size_t i = 0; i < cached.size(); ++ i) {
			...
			// STAGE-8 PORT MOD: null guard — trimmed new_from_type returns nullptr for unported patterns.
			if (fill) { cached[i] = fill->use_bridge_flow(); delete fill; }
		}
	}
...
{
    // STAGE-8 PORT MOD: body stubbed. Original builds an ExtrusionEntityCollection using
    // print_object_config (has()/option()->getBool()), Flow and _create_gap_fill (PrintObjectConfig
    // machinery, stubbed out). The WASM port calls fill_surface() (Polylines) directly instead.
    (void)surface; (void)params; (void)out;
}
...
void Fill::_create_gap_fill(const Surface* surface, const FillParams& params, ExtrusionEntityCollection* out){
    // STAGE-8 PORT MOD: body stubbed. Original gap-fill reads print_object_config (gftNowhere etc.)
    // + Flow + Arachne to append ExtrusionEntities. Port uses fill_surface(); gap-fill unused here.
    (void)surface; (void)params; (void)out;
}
