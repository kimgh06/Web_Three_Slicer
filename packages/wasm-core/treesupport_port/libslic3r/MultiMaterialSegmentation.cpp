...
#include "TriangleMeshSlicer.hpp"   // slice_mesh_slabs: the top/bottom shell projection
#include "Flow.hpp"                 // rounded_rectangle_extrusion_spacing (layer_color_stat)
#include "Geometry/VoronoiVisualUtils.hpp"
#include "Geometry/VoronoiUtils.hpp"
#include "MutablePolygon.hpp"
#include "Utils.hpp"          // next_idx_modulo
            ...
            if (Slic3r::sqr(v1.dot(v2)) > cos_threshold2 * v1_sqr_norm * v2.squaredNorm()) {
                ...
                if (painted_lines_set.find(*it_contour_and_segment) == painted_lines_set.end()) {
                    if (grid_line.distance_to_squared(line_to_test.a) < append_threshold2 ||
                        ...
                        {
                            std::lock_guard<std::mutex> lock(painted_lines_mutex);
...
// Model.hpp's own value (Model.hpp:1789). Copied rather than included: that header pulls in the whole model tree,
// and this is a single float the port would otherwise have no other reason to reach for.
...
static inline std::vector<std::vector<ExPolygons>> segmentation_top_and_bottom_layers(const std::vector<ExPolygons>  &input_expolygons,
                                                                                      ...
                                                                                      const std::function<void()>    &throw_on_cancel_callback)
{
    ...
    const int max_top_layers    = shell.top_shell_layers;
    const int max_bottom_layers = shell.bottom_shell_layers;
    const int granularity       = std::max(1, std::max(max_top_layers, max_bottom_layers) - 1);
    ...
    if (max_top_layers > 0 || max_bottom_layers > 0) {
        {
                const Transform3d volume_trafo = Transform3d::Identity();
                for (size_t extruder_idx = 0; extruder_idx < num_facets_states; ++extruder_idx) {
                    const indexed_triangle_set painted = painted_facets_of_state(extruder_idx, /* strict */ true);
#ifdef MM_SEGMENTATION_DEBUG_TOP_BOTTOM
                    {
                        static int iRun = 0;
    ...
    // Upstream maximises these over every PrintRegion of the layer, because a layer can hold regions printed by
    // different nozzles with different shell counts and line widths. The kernel has one region and one nozzle, so
    // every one of those maxima collapses to the single value the caller passes in — the statistic is the same
    // number, just without a search. layer_idx/color_idx stay in the signature: the call sites index by them and
    // this keeps the diff against upstream readable.
    auto layer_color_stat = [&shell](const size_t /* layer_idx */, const size_t /* color_idx */) -> LayerColorStat {
        LayerColorStat out;
        const double outer_wall_line_width = shell.outer_wall_line_width;
        out.extrusion_width     = float(outer_wall_line_width);
        out.top_shell_layers    = shell.top_shell_layers;
        out.bottom_shell_layers = shell.bottom_shell_layers;
        // Gap fill disabled (the kernel has none) -> upstream's second branch: two lines slightly overlapping.
        out.small_region_threshold = float(outer_wall_line_width) +
            0.7f * Flow::rounded_rectangle_extrusion_spacing(float(outer_wall_line_width), shell.layer_height);
        out.small_region_threshold = scaled<float>(out.small_region_threshold * 0.5f);
        out.extrusion_spacing = Flow::rounded_rectangle_extrusion_spacing(float(outer_wall_line_width), shell.layer_height);
        out.num_regions = 1;
...
// =====================================================================================================================
// The kernel's driver, in place of upstream's segmentation_by_painting(PrintObject, …).
//  Everything above this line is upstream's algorithm, unchanged. What upstream's driver does and this one does not:
//   · walk print_object.layers() for the contour   -> the kernel passes its own sliced contour in (layer_slices)
//   · walk model_object()->volumes for the paint   -> the kernel merges its objects into one mesh before slicing, and
//                                                     the selector holds the paint for exactly that mesh
//   · apply print_object.trafo() * mv->get_matrix() and subtract center_offset()
//                                                  -> selector and slicer share one coordinate frame already
//                                                     (bindings.cpp selector_prepare: XY as-is, Z seated), so every
//                                                     transform here is the identity and is dropped rather than
//                                                     carried as a no-op that could silently stop being one.
// =====================================================================================================================
std::vector<std::vector<ExPolygons>> mm_segmentation_by_painting(const std::vector<ExPolygons>  &layer_slices,
                                                                 ...
                                                                 const std::function<void()>    &throw_on_cancel_callback)
{
    const size_t num_layers = std::min(layer_slices.size(), zs.size());
    if (num_layers == 0 || num_facets_states < 2)
        return {};
    ...
    // Merge the layer contour and remove small holes. Upstream unions the LayerRegions of the layer; the kernel has
    // one region per layer, so the union is over that one contour — the expand/shrink pair is kept because it is what
    // welds very close polygons, and the simplify pair is what keeps Voronoi off self-intersecting input.
    BOOST_LOG_TRIVIAL(debug) << "MM segmentation - slices preprocessing - begin";
    tbb::parallel_for(tbb::blocked_range<size_t>(0, num_layers), [&layer_slices, &input_expolygons, &throw_on_cancel_callback](const tbb::blocked_range<size_t> &range) {
        for (size_t layer_idx = range.begin(); layer_idx < range.end(); ++layer_idx) {
            throw_on_cancel_callback();
            ExPolygons ex_polygons;
            for (const ExPolygon &expolygon : layer_slices[layer_idx])
    ...
    BOOST_LOG_TRIVIAL(debug) << "MM segmentation - slices preprocessing - end";

    std::vector<BoundingBox> layer_bboxes(num_layers);
    for (size_t layer_idx = 0; layer_idx < num_layers; ++layer_idx) {
        throw_on_cancel_callback();
        layer_bboxes[layer_idx] = get_extents(input_expolygons[layer_idx]);
    }

    for (size_t layer_idx = 0; layer_idx < num_layers; ++layer_idx) {
        throw_on_cancel_callback();
        // A layer that sliced to nothing (above the model, or a degenerate cut) has no contour to paint on, and
        // get_extents of an empty set is the inverted bbox — feeding that to set_bbox/create is what turns a
        // harmless empty layer into an out-of-range grid.
        if (input_expolygons[layer_idx].empty())
            continue;
        BoundingBox bbox = layer_bboxes[layer_idx];
        // Projected triangles could belong to polygons printed in the previous or the next layer, so the grid has to
        // cover its neighbours too (upstream's `layer_idx > 1` bound is kept as-is).
        if (layer_idx > 1 && !input_expolygons[layer_idx - 1].empty()) bbox.merge(layer_bboxes[layer_idx - 1]);
        if (layer_idx + 1 < num_layers && !input_expolygons[layer_idx + 1].empty()) bbox.merge(layer_bboxes[layer_idx + 1]);
    ...
    for (size_t extruder_idx = 1; extruder_idx < num_facets_states; ++extruder_idx) {
        throw_on_cancel_callback();
        const indexed_triangle_set custom_facets = painted_facets_of_state(extruder_idx, /* strict */ false);
        if (custom_facets.indices.empty())
            continue;
        tbb::parallel_for(tbb::blocked_range<size_t>(0, custom_facets.indices.size()), [&custom_facets, &zs, &edge_grids, &input_expolygons, &painted_lines, &painted_lines_mutex, &extruder_idx](const tbb::blocked_range<size_t> &range) {
            for (size_t facet_idx = range.begin(); facet_idx < range.end(); ++facet_idx) {
                ...
                for (int p_idx = 0; p_idx < 3; ++p_idx) {
                    facet[p_idx] = custom_facets.vertices[custom_facets.indices[facet_idx](p_idx)];
                    max_z        = std::max(max_z, facet[p_idx].z());
                    min_z        = std::min(min_z, facet[p_idx].z());
                }

                // A horizontal facet cuts no slicing plane at all — it is the top/bottom shell projection that
                // carries it, which is why that pass is not optional for painting on flat faces.
                ...
                auto first_layer = std::upper_bound(zs.begin(), zs.end(), float(min_z - EPSILON));
                auto last_layer  = std::upper_bound(zs.begin(), zs.end(), float(max_z + EPSILON));
                // Upstream decrements last_layer unconditionally. It is safe there because a painted facet always sits
                // inside the sliced range; here the selector's mesh and the slicing planes are computed separately, so
                // a facet entirely below the first plane would decrement begin() — an iterator underflow, which on
                // wasm32 is a raw trap rather than a diagnosable fault. Both ends are checked instead.
                if (first_layer == zs.end() || last_layer == zs.begin())
                    continue;
                --last_layer;

                for (auto layer_it = first_layer; layer_it <= last_layer; ++layer_it) {
                    const size_t layer_idx = size_t(layer_it - zs.begin());
                    const float  slice_z   = *layer_it;
                    if (input_expolygons[layer_idx].empty() || is_less(slice_z, facet[0].z()) || is_less(facet[2].z(), slice_z))
                        continue;

                    // https://kandepet.com/3d-printing-slicing-3d-objects/
                    float t            = (slice_z - facet[0].z()) / (facet[2].z() - facet[0].z());
                    Vec3f line_start_f = facet[0] + t * (facet[2] - facet[0]);
                    Vec3f line_end_f;

                    // When one side of a triangle coincides with the slice_z.
                    if ((is_equal(facet[0].z(), facet[1].z()) && is_equal(facet[1].z(), slice_z))
                        || (is_equal(facet[1].z(), facet[2].z()) && is_equal(facet[1].z(), slice_z))) {
                        line_end_f = facet[1];
                    } else if (facet[1].z() > slice_z) {
                        // [P0, P2] and [P0, P1]
                        float t1   = (slice_z - facet[0].z()) / (facet[1].z() - facet[0].z());
                        line_end_f = facet[0] + t1 * (facet[1] - facet[0]);
                    } else {
                        // [P0, P2] and [P1, P2]
                        float t2   = (slice_z - facet[1].z()) / (facet[2].z() - facet[1].z());
                    ...
                    // The painted line can fall outside the grid's bounding box (the grid is built from the contour,
                    // the line from the mesh). Clip it rather than let the visitor walk out of the grid.
    ...
    BOOST_LOG_TRIVIAL(debug) << "MM segmentation - layers segmentation - begin";
    tbb::parallel_for(tbb::blocked_range<size_t>(0, num_layers), [&edge_grids, &input_expolygons, &painted_lines, &segmented_regions, &num_facets_states, &throw_on_cancel_callback](const tbb::blocked_range<size_t> &range) {
        for (size_t layer_idx = range.begin(); layer_idx < range.end(); ++layer_idx) {
            ...
            if (color_poly.empty() || color_poly.front().empty())
                continue;
            if (has_layer_only_one_color(color_poly)) {
                // The whole layer is one colour — no Voronoi diagram needed to find a boundary that is not there.
    ...
    BOOST_LOG_TRIVIAL(debug) << "MM segmentation - layers segmentation - end";
    throw_on_cancel_callback();

    if (segmentation_max_width > 0.f) {
        cut_segmented_layers(input_expolygons, segmented_regions, float(scale_(segmentation_max_width)), 0.f, throw_on_cancel_callback);
        throw_on_cancel_callback();
    }

    // The first index is the extruder number (the default extruder included), the second one the layer number.
    std::vector<std::vector<ExPolygons>> top_and_bottom_layers;
    if (include_top_and_bottom_layers == IncludeTopAndBottomLayers::Yes) {
        top_and_bottom_layers = segmentation_top_and_bottom_layers(input_expolygons, zs, painted_facets_of_state, num_facets_states, shell, throw_on_cancel_callback);
        throw_on_cancel_callback();
    }

    return merge_segmented_layers(segmented_regions, std::move(top_and_bottom_layers), num_facets_states, throw_on_cancel_callback);
}
