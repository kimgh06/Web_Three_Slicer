...
The upstream OrcaSlicer sources live in `../slicer/`, and the reverse-engineered kernel, SDK and components live in `../packages/`
(with zero build or runtime dependency on `../slicer/`). Guide: [`GUIDE.md`](GUIDE.md), specs: [`SPECS.md`](SPECS.md).
...
Regenerate: `python3 web/extract_all.py` (paths are derived from `__file__` — the sources are in `../slicer/`, the output goes to `web/`).
...
- Location: `reverse_engineering/viewer/` (Vite + React 18 + react-router v7 + three.js, statically importing the 4 JSON files above).
- Run: `cd reverse_engineering/viewer && npm i && npm run dev` -> opens in the browser with hash routing.
...
- Location: `reverse_engineering/wasm-core/`.
- Build: `bash reverse_engineering/wasm-core/build.sh` (requires emscripten, including the `EMSDK_PYTHON` export).
  **From stage 7 it also needs brew `boost` + `eigen`** (the header-only voronoi/Eigen). The output `slicer_core.js`
  (SINGLE_FILE=1 -> the wasm inlined as base64) is committed in `viewer/src/slicer/`, so **using just the viewer works
  without emscripten/boost/eigen**.
- Self-tests: `node reverse_engineering/wasm-core/test.mjs` (cube + ASCII + overhang table + cylinder + thin
...
- **The schema cross-check = the build-based dump realized** (guide §11.1) — a function that dumps WASM's `print_config_def` to JSON
  (embind `dump_schema_json`) -> node generates `reverse_engineering/config-schema-builddump.json` (817 entries) ->
  `reverse_engineering/compare_schema.mjs` compares the fields against the existing regex-based `config-schema.json` (907). **Result**:
