...
> **스크린샷·테스트 wasm 참고**: 아래 단계별 기록에 `wasm-core/sNN_*.png`·`stageNN_*.png` 같은 검증
> 스크린샷 경로가 인용되지만, 이 파일들은 저장소에서 제외됐다(6.6MB). 테스트용 emscripten 산출물
> (`wasm-core/**/test_*.wasm`, 6.5MB)도 `build.sh` 로 재생성 가능한 빌드 결과물이라 제외했다.
> 둘 다 `.gitignore` 에 있고, 초기 커밋 이력에는 남아 있다. 인용은 당시 검증이 실제로 수행됐다는
> 기록으로 읽으면 된다.
