...
const count5=(r,xr,zr)=>{let n=0;for(const L of r.layers){const z=L.z;if(z<zr[0]||z>=zr[1])continue;const p=L.paths;if(!p)continue;for(let k=0;k<p.length;k+=8){if(p[k+3]===5){const mx=(p[k]+p[k+4])/2;if(mx>=xr[0]&&mx<xr[1])n++}}}return n}
...
for(const style of ['grid','tree_lite']){
  ...
  ok(count5(rk,[-3,3],[0,15])===0, `${style} cantilever: no support over/in pillar solid`)
