...
// (stage 17) role_to_string/string_to_role are provided by the MAIN build as single-function extracts
// (role_to_string.cpp + extrusion_role_helper.cpp). The treesupport group links against the main, so it
// must NOT redefine them (ODR duplicate). The tree pipeline never emits role strings; guard them out
// only for the treesupport group (-DTS_BRIDGE_EXCLUDE_ROLE_FNS). Standalone link test keeps them.
#ifndef TS_BRIDGE_EXCLUDE_ROLE_FNS
...
#endif // TS_BRIDGE_EXCLUDE_ROLE_FNS
