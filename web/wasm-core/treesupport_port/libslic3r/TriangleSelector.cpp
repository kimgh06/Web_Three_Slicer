...
// (stage 19 port) "Model.hpp" is a vestigial include here — TriangleSelector.cpp references NO Model/
// ModelVolume/ModelObject symbol (verified by grep); EnforcerBlockerType is defined in TriangleSelector.hpp
// itself. The real Model.hpp pulls the full SLA/CustomGCode/Emboss graph the kernel has no use for, so it
// is dropped for the isolated port build (documented adapter change; original logic untouched).
#include "Geometry.hpp"   // (stage 19 port) was reached transitively via Model.hpp; TriangleSelector.cpp uses Geometry::
