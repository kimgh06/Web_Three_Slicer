...
// (stage 16) boost/format.hpp umbrella: several TUs (BuildVolume/Flow/Config/PrintConfig/TreeSupport)
// use boost::format for log/error messages and upstream reached it transitively via Semver.hpp, whose
// port stub (boost-free, to dodge the flaky boost/optional lookup) no longer provides it. libslic3r.h
// is included by Point.hpp -> effectively every TU, so this restores it once, deterministically.
